...
class EbayShopifyImporter:
    ...
    def _fallback_load_all_products(self, search_category: str, keywords_dict: dict) -> List[Dict]:
        """
        Fallback plan: fetch all products and use score-based selection.

        This method is called when the primary endpoint fails.
        Uses the new scoring system instead of hard filtering.
        """
        ...
        try:
            ...
            # Use the new score-based selection (handles everything internally)
            # No need for separate filtering - _select_best_products now does:
            # 1. Minimal hard filter (product_type)
    ...
    # ═══════════════════════════════════════════════════════════════════
    # Product Filtering and Selection - SCORE-BASED SYSTEM
    # ═══════════════════════════════════════════════════════════════════
    ...
    def _calculate_relevance_score(self, product: Dict, keywords_dict: dict, relaxed_mode: bool = False) -> Tuple[float, Dict]:
        ...
        - product_type == "case" → +50 points (strong match)
        - product_type == "accessory" → +20 points (medium match)
        - product_type == "other" → +5 points (weak match)
        ...
        - Negative keywords → -15 points each (max -45 penalty, NOT rejection)

        Args:
            product: Product dictionary
            keywords_dict: Keywords configuration
            relaxed_mode: If True, reduce penalties (for fallback)
        ...
        # ═══════════════════════════════════════════════════════════════
        # 1. PRODUCT TYPE SCORING (Strong signal)
        # ═══════════════════════════════════════════════════════════════
        ...
        # Note: other types get 0 points but are NOT rejected

        score += breakdown['product_type']

        # ═══════════════════════════════════════════════════════════════
        # 2. CASE/COVER KEYWORDS IN TITLE (Strong signal for phone cases)
        # ═══════════════════════════════════════════════════════════════
        ...
        for kw in case_keywords:
            if kw in title:
                case_matches += 1
                if case_matches >= 2:  # Cap at 2 matches
                    break
        breakdown['case_keywords'] = min(case_matches * 30, 60)
        score += breakdown['case_keywords']

        # ═══════════════════════════════════════════════════════════════
        # 3. PHONE BRAND/MODEL KEYWORDS (Strong signal)
        # ═══════════════════════════════════════════════════════════════
        brand_keywords = [
        ...
        # ═══════════════════════════════════════════════════════════════
        # 5. POSITIVE KEYWORDS SCORING (from keywords_dict)
        # ═══════════════════════════════════════════════════════════════
        ...
        # ═══════════════════════════════════════════════════════════════
        # 6. NEGATIVE KEYWORDS PENALTY (Small penalty, NOT rejection)
        # ═══════════════════════════════════════════════════════════════
        negative = keywords_dict.get('negative', [])
        penalty_multiplier = 0.5 if relaxed_mode else 1.0  # Reduce penalty in relaxed mode
    ...
    def _apply_minimal_hard_filter(self, products: List[Dict], allowed_types: List[str] = None) -> List[Dict]:
        ...
        This is the ONLY hard rejection allowed:
        - Reject products where product_type is NOT in allowed types

        Args:
            products: List of products
            allowed_types: List of allowed product_type values
        ...
        for product in products:
            ...
            # MINIMAL HARD FILTER: only reject if product_type exists AND is not in allowed list
    ...
    def _score_all_products(self, products: List[Dict], keywords_dict: dict, relaxed_mode: bool = False) -> List[Dict]:
        ...
        Args:
            ...
            keywords_dict: Keywords configuration
            relaxed_mode: If True, reduce penalties

        Returns:
            List of scored product dictionaries
        """
        if not products:
            return []
    ...
    def _select_best_products(self, products: List[Dict], keywords_dict: dict = None) -> List[Dict]:
        """
        Select best products using SCORE-BASED RANKING (not hard filtering).

        Strategy: Ranking > Filtering, Best Match > Perfect Match

        Steps:
        1. Apply minimal hard filter (only reject completely wrong product_type)
        2. Score all products based on relevance + quality
        3. Select top N products by combined score
        4. Smart fallback if 0 products (relax criteria and retry)
        """

        print(f"\n[INFO] Analyzing {len(products)} products to select best {self.max_products}...")

        # ═══════════════════════════════════════════════════════════════════
        # STEP 1: Minimal Hard Filter (only product_type check)
        # ═══════════════════════════════════════════════════════════════════
        allowed_types = ['case', 'accessory', 'other']
        ...
        if len(scored_products) == 0:
            ...
            if len(scored_products) == 0:
                # Fallback 2: Expand allowed product types
            ...
            if len(scored_products) == 0:
                # Fallback 3: Use original products with relaxed scoring (last resort)
        ...
        for i, item in enumerate(selected, 1):
            ...
            # Show relevance breakdown if available
        ...
        # Final safety check - NEVER return empty list
    ...
    def _rewrite_title(self, title: str) -> str:
        ...
        Strategy:
        1. Extract brand/model (iPhone, Samsung Galaxy, etc.)
        2. Extract product type (Case, Cover, Charger, etc.)
        3. Extract key features (Shockproof, Slim, Clear, etc.)
        4. Build clean professional title: "[Feature] [Product Type] for [Brand/Model]"
        ...
        # ═══════════════════════════════════════════════════════════════
        brand_patterns = {
            # iPhone patterns (order matters - specific first)
        ...
        # ═══════════════════════════════════════════════════════════════
        # 3. EXTRACT KEY FEATURES (max 2 features)
        # ═══════════════════════════════════════════════════════════════
        features = {
        ...
        for feature_name, pattern in features.items():
            if re.search(pattern, title_lower):
                ...
                if len(detected_features) >= 2:  # Max 2 features
                    break
        ...
        if new_title:
            # Title case but keep "for" lowercase
            words = new_title.split()
            words = [w.capitalize() if w.lower() != 'for' else 'for' for w in words]
            new_title = ' '.join(words)

        # Fallback: if new title is too short, use cleaned original
        if len(new_title) < 10:
