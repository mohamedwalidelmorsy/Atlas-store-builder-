...
def success():
    ...
    store_password = request.args.get('store_password', '')
...
def run_automation_background(store_id, user_data):
    ...
    try:
        ...
        update_entry_status(store_id, {
            ...
            'store_url': store_data['store_url'],
            'admin_url': store_data.get('admin_url'),
            'store_password': store_data.get('store_password', ''),
...
def api_create_store():
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['client_name', 'store_name', 'email', 'product_category']
...
def api_get_store_status(store_id):
    ...
    if entry.get('status') == 'completed':
        ...
        response['store_name'] = entry.get('user_data', {}).get('store_name', '')
        response['customer_email'] = entry.get('user_data', {}).get('email', '')
        response['products_imported'] = entry.get('products_imported', 0)
        response['products_count'] = entry.get('products_imported', 0)
        response['store_password'] = entry.get('store_password', '')
